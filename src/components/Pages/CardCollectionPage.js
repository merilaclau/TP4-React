...
const CardCollectionPage = () => {
    ...
    useEffect(() => {
        ...
        getMoviesSeriesByPage("movie", "popular", currentPage).then(data => setData(data));
        console.log(data);
    }, []);
    
    const {pagination, prevPage: goToPrevPage, nextPage: goToNextPage, changePage} = usePagination(data.total_pages, data.page)
        ...
        <div>
            ...
            <nav className="pagination">
                ...
                */}
                ...
                <Link to={{ pathname: '/'}} /*className="pagination-next"*/ onClick={nextPage}>{'>'}</Link>
            </nav>
        </div>
